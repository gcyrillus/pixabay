<?php
	...
	**/		
	...
	if(isset($_GET['p'])&& $_GET['p'] !== 'Pixabay' ) {goto end;}
	
	# on charge la class du plugin pour y accéder
	$plxMotor = plxMotor::getInstance();
	$plxPlugin = $plxMotor->plxPlugins->getInstance( 'Pixabay'); 
	
	# On vide la valeur de session qui affiche le Wizard maintenant qu'il est visible.
	if (isset($_SESSION['justactivatedPixabay'])) {unset($_SESSION['justactivatedPixabay']);}
	...
	#affichage
?>
<link rel="stylesheet" href="<?= PLX_PLUGINS ?>Pixabay/css/wizard.css" media="all" />
<input id="closeWizard" type="checkbox">
<div class="wizard">	
	<div class="container">	
		<div class='title-wizard'>
			<h2><?= $plxPlugin->aInfos['title']?><br><?= $plxPlugin->aInfos['version']?></h2>
			<img src="<?php echo PLX_PLUGINS. 'Pixabay'?>/icon.png">
		...
		<form action="parametres_plugin.php?p=<?php echo 'Pixabay' ?>"  method="post">
			<div role="tab-list">		
				<div role="tabpanel" id="tab1" class="tabpanel">
					...
					<p>Cette extension vous permet d'accéder à une banque de plus d'<b  style="font-family:cursive;color:crimson;font-variant:small-caps;">1 million</b> d'images sur PiXabay.com.</p>
					...
					<p><img src='/plugins/Pixabay/img/pixabaySearch.png' alt='champ de recherche'></p>
				</div>	
				...
				<div role="tabpanel" id="tab3" class="tabpanel hidden">
					<h2>préparation</h2>
					<p style="grid-column:2;grid-row:1/4;max-width:25vw;"><img src='/plugins/Pixabay/img/rejoindre.png' alt='champ de recherche'></p>
				...
				<div role="tabpanel" id="tabEnd" class="tabpanel hidden title">
					<h2>Finaliser</h2>
					<p></p>
					<p>Votre N° de clé: <?php plxUtils::printInput('apikey',$plxPlugin->apikey,'text','30-50') ?></p>
				</div>		
				...
				<p class="idConfig">
				<?php
				if(file_exists(PLX_PLUGINS. 'Pixabay/admin.php')) {echo ' 
				<a href="/core/admin/plugin.php?p= Pixabay">Page d\'administration '. basename(__DIR__ ).'</a>';}
				if(file_exists(PLX_PLUGINS. 'Pixabay/config.php')) {echo ' 	<a href="/core/admin/parametres_plugin.php?p=Pixabay">Page de configuration  Banqued\'image</a>';}
				?>
				<label for="closeWizard"> Fermer </label>
				</p>	
				</div>	
				<script src="<?= PLX_PLUGINS ?>Pixabay/js/wizard.js"></script>
				</div>
				<?php end: // FIN! ?>				
